...
namespace EventEdu.Persistence.Services
{
	public class HeroSectionService : IHeroSectionService
	{
		...
		public async Task<(List<HeroSection>,HeroSectionDetails)> GetHeroSectionAll()
		{
			var languages = _contextAccessor?.HttpContext?.Request?.Headers["accept-language"].FirstOrDefault().Split(',').FirstOrDefault();
			var language = await _languageReadRepository.GetByIsoCodeAsync(languages);
			var heroSection = await _sliderReadRepository.GetAll().ToListAsync();
			var heroSectionList= new HeroSectionDetails();
			foreach (var hs in heroSection)
			{
				var heroSectionDetails = await _sliderDetailReadRepository.GetAll().FirstOrDefaultAsync(a => a.LanguageId == language.Id && a.HeroSectionId==hs.Id);

				heroSectionList= heroSectionDetails;
			}
			return (heroSection, heroSectionList);
		}
	}
